...
1. **Automated unit tests** — no framework wired yet. Vitest is a
   natural fit (module bundler is already webpack, jsdom lightweight).
   Priority: `outing-session.js` haversine + snapshot logic, `offline.js`
   sync flow, `NearMeButton.computeBbox`.
